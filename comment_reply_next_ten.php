						...
						<div class="span1 reply-indicator">
							...
							<?php } ?> 
								...
								<?php if(($_SESSION['user_id'] != $imagecomment['user_id']) && ($imagecomment['is_reported'] < 1) && ($_SESSION['user_id'] != $imagecomment[2])){?>
									...
									<a href="javascript:void(0);" class="likecomment" id="likecomment" data-commentid="<?php echo $imagecomment['commentid']; ?>"><i class="icon-thumbs-up"></i></a>
									<a href="javascript:void(0);" class="dislikecomment" id="dislikecomment" data-commentid="<?php echo $imagecomment['commentid']; ?>"><i class="icon-thumbs-down"></i></a>
								...
								if($_SESSION['user_id'] == $imagecomment[2]){?>
									...
									<?php if($imagecomment['is_reported'] >= 1){?>
										...
										<span class="hide-reported<?php echo $imagecomment['commentid'];?>">
								...
								<?php if($imagecomment['is_reported'] >= 1 ){?>
									...
									<p class="comments-layout hide-reported<?php echo $imagecomment['commentid']; ?>"><?php echo $imagecomment['comment']; ?></p>
									
									<script text="text/javascript">$('.hide-reported<?php echo $imagecomment['commentid']; ?>').hide();</script>
									...
									<span class="reply-comment" id="<?php echo $imagecomment['commentid']; ?>">
										<div class="row-fluid">
											<div class="span12">
												...
												<button class="btn btn-mini submit-reply-button-modal" data-commentid="<?php echo $imagecomment['commentid']; ?>" data-image_id="<?php echo $imagecomment['image_id']; ?>"><strong>Submit</strong></button>
												<strong><span id="comment-counter<?php echo $imagecomment['commentid']; ?>"></span></strong>
												<?php var_dump($imagecomment['image_id']); ?>
