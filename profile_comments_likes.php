<span class="page7">
	<?php foreach($comments_likes as $comment){
	...
	<span id="comment<?php echo $comment['commentid']; ?>">
		...
		<div class="row-fluid" id="commentContainer-inner">
			<div class="span12">
				...
				<div class="span2">
					<div class="comment-image-container nailthumbs-likes">
					...
					if($_SESSION['user_id'] == $comment[2]){?>
						...
						<?php }} else { 
							...
							<span class="hide-reported hide-reported<?php echo $comment['commentid'];?>">
								...
								<a href="javascript:void(0);" class="show-hide-reply-comment-like" data-commentid="<?php echo $comment['commentid']; ?>"><small>reply</small></a>
								<a href="javascript:void(0);" class="hide-comment" data-commentid="<?php echo $comment['commentid']; ?>"><small>hide</small></a>
						...
						<?php if($_SESSION['user_id'] != $comment[2]){?>
							...
							<span class="reply-comment" id="like<?php echo $comment['commentid']; ?>">
								<div class="row-fluid">
									<div class="span12">
										<textarea class="<?php echo $comment['commentid']; ?> like<?php echo $comment['commentid']; ?>" id="reply-textarea" placeholder="Comment..."></textarea><br>
											<button class="btn btn-mini profile-submit-reply-button" data-commentid="<?php echo $comment['commentid']; ?>" data-user_id="<?php echo $comment['user_id']; ?>" data-arbitrary="1"><strong>Submit</strong></button>
...
<?php }}?>
	...
	<?php $comment_pages = $count_comments_likes[0]/10;
	...
	if($comment_pages >= 1){
		...
		for($i = 2; $i <= ($comment_pages + 1); $i++){?>
			...
			<span class="page7">
				<span class="comments-likes-page<?php echo $i;?>">
			...
			<div class="pagination pagination-mini pagination-centered">
				<ul class="profile-comment-likes-nav">
				
					<?php for($i = 1; $i <= ($comment_pages + 1); $i++){ ?>
					
						<li class="<?php if($i == 1){?> done active<?php }?>"><a href="javascript:void(0);" class="profile-comment-next-likes-page<?php if($i == 1){?> done<?php } ?>" 
